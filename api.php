...
/*
======================================================================
 API エンドポイント集約 (api.php)

 概要
 - 掲示板の投稿/返信/削除、プロフィール参照/更新のAPIを提供
 - フロントエンドから fetch() で呼び出し、JSON を返す

 リクエスト分岐
 [GET]
 - action=get_my_profile: ログイン中ユーザーのプロフィール取得
 - action=get_profiles: プロフィール一覧の取得（ページング・並び替え）
 - id=...: 自分の投稿1件の取得（編集用・本人のみ）
 - parent_id=...: 指定スレッドの返信一覧取得（古い順）
 - 上記以外: 親スレッド一覧の取得（ページング・並び替え、返信数付き）

 [POST]
 - action=update_profile: 自分のプロフィールを新規作成/更新（UPSERT）
 - action=delete: 自分の投稿を削除（本人のみ）
 - id=...: 自分の投稿本文を編集（本人のみ）
 - parentpost_id=...: 返信の新規作成
 - 上記以外: 親スレッド（新規スレッド）の作成

 主な処理/関数
 - require_login(): 未ログインのアクセスを拒否/誘導
 - PDO + プリペアドステートメントでDB操作（SQLインジェクション対策）
 - 入力検証: 必須項目チェック、IDの整数化、並び替え列のホワイトリスト制限
 - 権限チェック: 投稿の編集/削除は投稿者本人のみ許可
 - レスポンス: すべてJSON（HTMLは返さない）
